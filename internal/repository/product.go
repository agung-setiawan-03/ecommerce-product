...
func (r *ProductRepo) InsertNewProduct(ctx context.Context, product *models.Product) error {
	...
	if err == nil {
		go func() {
			...
			if err := r.Redis.Del(ctx, constants.RedisKeyProduct).Err(); err != nil {
				helpers.Logger.Warn("failed to delete redis with key: ", constants.RedisKeyProduct, err)
			}
			if err := r.Redis.Set(ctx, fmt.Sprintf(constants.RedisKeyProductDetail, product.ID), string(jsonData), time.Hour*24).Err(); err != nil {
				helpers.Logger.Warn("failed to insert redis with key: ", fmt.Sprintf(constants.RedisKeyProductDetail, product.ID), err)
			}
		}()
	}
...
func (r *ProductRepo) UpdateProduct(ctx context.Context, productID int, newData map[string]interface{}) error {
	...
	go func() {
		...
		if err := r.Redis.Del(ctx, constants.RedisKeyProduct).Err(); err != nil {
			helpers.Logger.Warn("failed to delete product in redis with key: ", constants.RedisKeyProduct, err)
		}
		if err := r.Redis.Del(ctx, fmt.Sprintf(constants.RedisKeyProductDetail, productID)).Err(); err != nil {
			helpers.Logger.Warn("failed to delete product in redis with key: ", constants.RedisKeyProduct, err)
		}
	}()
	return nil
...
func (r *ProductRepo) UpdateProductVariant(ctx context.Context, variantID int, newData map[string]interface{}) error {
	...
	go func() {
		...
		if err := r.Redis.Del(ctx, fmt.Sprintf(constants.RedisKeyProductDetail, variant.ProductID)).Err(); err != nil {
			helpers.Logger.Warn("failed to delete product in redis with key: ", constants.RedisKeyProduct, err)
		}
	}()
	return nil
...
func (r *ProductRepo) GetAllProducts(ctx context.Context, page int, limit int) ([]models.Product, error) {
	offset := (page - 1) * limit
	products := []models.Product{}

	// Get data in Redis
	productStr, err := r.Redis.Get(ctx, constants.RedisKeyProduct).Result()
	if err == nil && productStr != "" {
		result := []models.Product{}
		if err := json.Unmarshal([]byte(productStr), &products); err != nil {
			helpers.Logger.Warn("failed to unmarshal the redis response get all products")
		}
		if page > 0 && limit > 0 {
			...
			helpers.Logger.Info("successfully get all products from redis cache")
			return products, nil
		}
	...
	go func() {
		...
		if err != nil {
			helpers.Logger.Warn("failed to get all products from cache: ", err)
			return
		}
		...
		err = r.Redis.Set(ctx, constants.RedisKeyProduct, string(jsonCacheProduct), time.Hour*24).Err()
		if err != nil {
...
func (r *ProductRepo) GetProductDetail(ctx context.Context, productID int) (models.Product, error) {
	...
	if err == nil && productStr != "" {
		if err := json.Unmarshal([]byte(productStr), &product); err != nil {
			helpers.Logger.Warn("failed to unmarshal the redis response get product detail")
		}

		helpers.Logger.Info("successfully product detail from redis cache")
		return product, nil
	}
